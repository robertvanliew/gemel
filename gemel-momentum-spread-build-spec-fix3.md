...
> **CHANGELOG vs. "fix 2" (the version you built from) — only these changed:**
> - **§8.3 revised** — the sort-to-top was user error, not a bug; kept only the eligibility rule (n/a ROC can never qualify) + NaN-to-bottom on sorts
> - **§8.7 NEW** — strike increments must come from real chains; missing chain shows "no data," never "untradeable"
> - **§9 NEW** — regime gate for the momentum playbook (92% of SPY's 1-yr high, checked monthly)
> - **Walkthrough step 1** — gate check added before Rank now
> Everything else is unchanged from the file you have.
...
**8.3 Missing-signal rows: eligibility, not sorting (revised — the "sorts to top" report was user error, a stray column click).**
Real issue kept: SPCX displayed ✓ fits-cap while having no ROC data at all. Rule: rows with missing 1-yr ROC are **ineligible as candidates regardless of every other gate** — no signal means nothing to rank; affordability without signal must never qualify. Minor polish: on any column sort, NaN/n-a values go to the bottom.
...
**8.7 Strike increments must come from real chains; "no data" ≠ "untradeable" (found 7/6 third run).**
With chains failing (liquidity all dashes), the spread builder fell back to price-bucket increment guesses (>$500 → 25w, >$1,500 → 50w) and confidently labeled names "untradeable" off invented contracts. Liquid high-priced names (META, GS, LLY, possibly MU) often have $5–10 LEAP strike increments in reality — some may genuinely fit the cap. Fixes: (a) minimum width derived only from actual strikes in a fetched chain — if no chain, no verdict; (b) when chain data is missing, the FITS CAP column shows **"no data"**, not "untradeable" — never convert a fetch failure into a confident conclusion (downstream fail-closed exclusion from candidates stays as is); (c) this makes 8.5 the blocking fix — nothing downstream is trustworthy until real chains load reliably.

---

## 9. Regime gate for the momentum playbook (NEW)

**Rule (one rule only, v1):** at the monthly re-rank, compute SPY's highest close over the trailing 252 trading days. If SPY's current close is **below 92% of that high** (more than 8% off the one-year high), the gate is CLOSED.

- Gate OPEN → normal session: candidates card populates, new entries allowed
- Gate CLOSED → **no new spreads this month.** Existing positions are NOT force-closed; they keep their normal exits (75% of max value / re-rank signal / <45 DTE warning). Candidates card shows: "Regime gate closed — SPY is X% below its 1-yr high. No new entries. Cash is a position."
- Displayed as a banner at the top of the Momentum page; computed at the monthly re-rank (state may refresh on page load, but it is a monthly decision input, never an intraday alarm)
- Provenance: mirrors the regime filter in the *certified* version of the source strategy, and parallels the credit side's existing SPY/50-EMA gate

**Explicitly not in the gate (v1):** equal-weight S&P (RSP) divergence, sector breadth, VIX, or any second signal. Breadth/equal-weight belong on the Today page as dashboard context only. A single unambiguous rule leaves no disagreement cases for discretion to exploit. A breadth filter may be promoted to a rule later **only** via backtest evidence (§6), never mid-cycle.

---

## Acceptance walkthrough (Julie's monthly session, ~30 min)

1. Open **Momentum spreads** page → regime gate banner first: OPEN → proceed; CLOSED → no new entries this month, review existing positions only
2. Click **Rank now**
3. Read top of list; illiquid and over-cap names already flagged; themes visible
4. Enter/adjust paper spreads on qualifying leaders (≤7 total, ≤2 per theme)
5. Any held name flagged by re-rank alert → close it
6. Weekly between sessions: 5-minute glance at position cards; close anything showing "Profit exit triggered"
7. After the cycle: review scorecard — adherence % first, P&L second
